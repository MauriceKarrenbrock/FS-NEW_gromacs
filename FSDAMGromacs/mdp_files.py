...
class BasicMdpFileMixIn(object):
    """MixIn class to create the basic part of a mdp file

    Methods
    --------
    create_basic_template

    TODO not all possible keys has been set
    """
    @staticmethod
    def create_basic_template(input_dictionary):
        """returns the template for a generic md mdp

        no check on the quality of the inputs is done

        input_dictionary : dict(str: value)
            the keys must be exacly as in the mpd file
            for missing keys a default will be used
            for unexpected keys they will be put at the end

        Returns
        ----------
        list(str)
            newline caracter missing
        """

        used_keys = set()

        def make_key_value_string_with_default(key, default_value):
            """helper function for create_basic_template method
            """

            used_keys.add(key)

            return f'{key}  =  {input_dictionary.get(key, default_value)}'

        generic_template = [
            '; VARIOUS PREPROCESSING OPTIONS',
            '; Preprocessor information: use cpp syntax.',
            '; e.g.: -I/home/joe/doe -I/home/mary/roe',
            make_key_value_string_with_default('include', ''),
            '; e.g.: -DPOSRES -DFLEXIBLE (note these variable names are case sensitive)',
            make_key_value_string_with_default(
                'define', ''), '', '; RUN CONTROL PARAMETERS',
            make_key_value_string_with_default(
                'integrator', 'md'), '; Start time and timestep in ps',
            make_key_value_string_with_default('tinit', 0),
            make_key_value_string_with_default('dt', 0.001),
            make_key_value_string_with_default('nsteps', 10000),
            '; For exact run continuation or redoing part of a run',
            make_key_value_string_with_default('init-step', 0),
            '; Part index is updated automatically on checkpointing (keeps files separate)',
            make_key_value_string_with_default(
                'simulation-part',
                1), '; mode for center of mass motion removal',
            make_key_value_string_with_default('comm-mode', 'Linear'),
            '; number of steps for center of mass motion removal',
            make_key_value_string_with_default(
                'nstcomm',
                100), '; group(s) for center of mass motion removal',
            make_key_value_string_with_default(
                'comm-grps', ''), '', '; TEST PARTICLE INSERTION OPTIONS',
            make_key_value_string_with_default(
                'rtpi', 0.05), '', '; OUTPUT CONTROL OPTIONS',
            '; Output frequency for coords (x), velocities (v) and forces (f)',
            make_key_value_string_with_default('nstxout', 10000),
            make_key_value_string_with_default('nstvout', 10000),
            make_key_value_string_with_default('nstfout', 10000),
            '; Output frequency for energies to log file and energy file',
            make_key_value_string_with_default('nstlog', 1000),
            make_key_value_string_with_default('nstcalcenergy', 100),
            make_key_value_string_with_default(
                'nstenergy',
                1000), '; Output frequency and precision for .xtc file',
            make_key_value_string_with_default('nstxtcout', 2000),
            make_key_value_string_with_default('xtc-precision', 1000),
            ...
            'tau-t                    = 0.2',
            make_key_value_string_with_default('ref-t',
                                               298.15), '; pressure coupling',
            ...
            make_key_value_string_with_default('continuation', 'no'),
        ...
        for key in input_dictionary.keys():
            ...
            if key not in used_keys:
                ...
                generic_template.append(
                    make_key_value_string_with_default(key, None))
